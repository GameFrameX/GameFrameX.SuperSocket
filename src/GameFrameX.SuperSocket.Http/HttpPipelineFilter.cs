...
public class HttpPipelineFilter : IPipelineFilter<HttpRequest>
{
    ...
    private static readonly ReadOnlyMemory<byte> _headerTerminator = new[] { (byte)'\r', (byte)'\n', (byte)'\r', (byte)'\n', };
    ...
    public HttpRequest Filter(ref SequenceReader<byte> reader)
    {
        if (_bodyLength == 0)
        {
            ...
            if (!reader.TryReadTo(out ReadOnlySequence<byte> pack, terminatorSpan, false))
            {
                return null;
            }
